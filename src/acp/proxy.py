...
class ACPProcess:
    ...
    async def start(self):
        ...
        await self._rpc("initialize", {"protocolVersion": 1, "clientInfo": {"name": "OpenMate", "version": "1.0.0"}})
    ...
    async def new_session(self, cwd: str = "/home/climbing") -> dict:
        ...
        if sid: self._default_session_id = sid
        return resp
        ...
        async with self._read_lock:
            ...
            while time.time() - start < 30:
                ...
                if msg and str(msg.get("id", "")) == msg_id:
                    if "error" in msg: raise Exception(msg["error"])
    ...
    async def _prompt_parts(self, parts: list[dict], session_id: str) -> dict:
        ...
        self._msg_id += 1
        msg_id = str(self._msg_id)
        request = {"jsonrpc": "2.0", "id": msg_id, "method": "session/prompt",
        ...
        async with self._read_lock:
            ...
            while time.time() - start < 60:
                try:
                    ...
                    if response and collected: break
                    if response: break
                    continue
                if not raw: break
    ...
    async def _cli(self, text: str) -> dict:
        try:
            ...
            return {"stopReason": "end_turn", "response_text": stdout.decode("utf-8", errors="replace").strip(), "source": "hermes-cli"}
        except asyncio.TimeoutError:
...
def get_acp_process() -> ACPProcess:
    global _acp
    if _acp is None: _acp = ACPProcess()
    return _acp
