...
async def update(tag_id: UUID, data: TagUpdate, user_id: UUID):
    ...
    if not fields:
        row = await pg_pool.fetchrow("SELECT * FROM tags WHERE id = $1 AND user_id = $2", tag_id, user_id)
        if not row:
...
async def delete(tag_id: UUID, user_id: UUID):
    result = await pg_pool.execute("DELETE FROM tags WHERE id = $1 AND user_id = $2", tag_id, user_id)
